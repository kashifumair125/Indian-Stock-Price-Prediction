...
def prediction_chart(df, test_idx, results, future_dates, future_preds, symbol):
    ...
    fig.add_vline(x=str(test_idx[0]), line_dash="dot",
                  line_color="rgba(255,255,255,0.3)",
                  annotation_text="Test period →", annotation_font_size=11)
...
def main():
    ...
    if last_sig == 1:
        badge, label = "badge-buy",  "BUY"
    elif last_sig == -1:
        badge, label = "badge-sell", "SELL"
    else:
        badge, label = "badge-hold", "HOLD"
    ...
    with col1:
        ...
        <div class="card" style="text-align:center;">
            ...
            <div style="font-size:2.5rem;margin:0.5rem 0;">
                <span class="badge {badge}" style="font-size:1.4rem;padding:0.4rem 1.2rem;">
                    {label}
                </span>
            </div>
            <div style="color:#64748b;font-size:0.85rem;">
                RSI: {rsi_now:.1f} &nbsp;|&nbsp;
                MACD: {'bullish' if macd_now > macd_sig else 'bearish'}
            </div>
        </div>
        """, unsafe_allow_html=True)
    with col2:
        buy_count  = (sigs == 1).sum()
        sell_count = (sigs == -1).sum()
        hold_count = (sigs == 0).sum()
        fig = px.pie(
            values=[buy_count, sell_count, hold_count],
            names=["Buy", "Sell", "Hold"],
            color_discrete_sequence=["#34d399", "#f87171", "#64748b"],
            hole=0.5,
        )
        fig.update_layout(template=DARK, height=220,
