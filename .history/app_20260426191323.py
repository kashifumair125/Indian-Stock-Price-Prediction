...
def main():
    ...
    if not run:
        ...
        header_html = """
        <div style="display:flex;padding:0.4rem 0;border-bottom:1px solid #111827;
                    font-size:0.68rem;font-weight:600;color:#3d4a6b;text-transform:uppercase;letter-spacing:0.07em;">
            <div style="width:10px;margin-right:10px;"></div>
            <div style="min-width:130px;">Symbol</div>
            <div style="flex:1;">Company</div>
            <div style="min-width:60px;text-align:center;">Sector</div>
            <div style="min-width:90px;text-align:right;">Price</div>
            <div style="min-width:80px;text-align:right;">Change</div>
            <div style="min-width:60px;text-align:right;">Signal</div>
        </div>"""
        rows_html = ""
        for msym, mname, msec in MOVERS:
            try:
                ...
                if not md.empty and len(md) >= 2:
                    ...
                    mp2 = md["Close"].iloc[-2]
                    mch = (mp - mp2) / mp2 * 100
                    dot_cls  = "dot-buy" if mch >= 0 else "dot-sell"
                    chg_cls  = "pos" if mch >= 0 else "neg"
                    arr      = "▲" if mch >= 0 else "▼"
                    ...
                    rows_html += f"""
                    <div class="trade-row">
                        <div class="trade-dot {dot_cls}"></div>
                        <div class="trade-sym">{msym}</div>
                        <div class="trade-name">{mname}</div>
                        <div style="min-width:60px;text-align:center;font-size:0.72rem;color:#3d4a6b;">{msec}</div>
                        <div class="trade-px">₹{mp:,.2f}</div>
                        <div class="trade-chg {chg_cls}">{arr} {abs(mch):.2f}%</div>
                        <div style="min-width:60px;text-align:right;">
                            <span class="badge {sig_cls}">{sig}</span>
                        </div>
                    </div>"""
                else:
                    rows_html += f'<div class="trade-row"><div class="trade-sym">{msym}</div><div class="trade-name">{mname}</div><div style="flex:1"></div><div class="trade-px">—</div></div>'
            except Exception:
                pass
